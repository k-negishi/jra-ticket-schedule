...
<template>
  ...
  <div v-if="isEnable" class="date-section">
    ...
    <div class="date-group">
      ...
      <div class="date-info">
        ...
        <p class="date-info-time">{{ preSaleEntryStart + ' 18:00 〜 ' + preSaleEntryEnd + ' 13:00' }}</p>
      </div>
      <div class="date-info">
        <p class="date-info-title">当選結果・購入開始日</p>
        <p class="date-info-time">{{ preSaleResultStart + ' 18:00 〜 ' + preSaleResultEnd + ' 13:00' }}</p>
      </div>
    </div>
    <!-- 一般抽選販売 -->
    <div class="date-group">
      ...
      <div class="date-info">
        <p class="date-info-title">抽選申込開始日</p>
        <p class="date-info-time">{{ generalSaleEntryStart + ' 18:00 〜 ' + generalSaleEntryEnd + ' 13:00' }}</p>
      </div>
      <div class="date-info">
        <p class="date-info-title">当選結果・購入開始日</p>
        <p class="date-info-time">{{ generalSaleResultStart + ' 18:00 〜 ' + generalSaleResultEnd + ' 13:00' }}</p>
      </div>
    </div>
    <!-- 残席先着販売 -->
    <div class="date-group">
  ...
  <div v-else>
    <p class="error-message">土曜日または日曜日を選択してください。<br>金杯・ホープフルS等には対応していません。</p>
  </div>
</template>
