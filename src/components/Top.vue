...
const calculationDate = (standardSubtractStart) => {
    const originDate = dayjs(inputDate.value)
    if (originDate.day() == 0) {
...
<template>
    ...
    <div v-else>
        <p class="error-message">土曜日か日曜日を選択してください</p>
    </div>
</template>
...
/* Date group style */
.date-group {
    @apply w-full md:w-1/3 px-2 mb-4;
}
...
/* Date info style */
.date-info {
    @apply p-4 bg-white border border-green-400 rounded shadow-md;
}
